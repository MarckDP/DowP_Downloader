...
class MainWindow(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        # Parte izquierda del frame (Contenedor)
        left_sub_frame = ctk.CTkFrame(container_frame, fg_color="transparent")
        left_sub_frame.pack(side="left")
        ctk.CTkLabel(left_sub_frame, text="Contenedor:").pack(side="left")
        self.recode_container_label = ctk.CTkLabel(left_sub_frame, text="-", font=ctk.CTkFont(weight="bold"))
        self.recode_container_label.pack(side="left", padx=5)

        # Parte derecha del frame (Consejo)
        gpu_info_label = ctk.CTkLabel(container_frame,
                                    text="Consejo: Elige un códec que coincida con tu GPU (NVENC=Nvidia, QSV=Intel, etc).",
                                    font=ctk.CTkFont(size=11, slant="italic"),
                                    text_color="gray")
        gpu_info_label.pack(side="right", padx=10)
    ...
    def cancel_operation(self):
        """Maneja la cancelación dejando los datos del análisis intactos para reintentar."""
        print("DEBUG: Botón de Cancelar presionado.")
        
        # 1. Señaliza la cancelación a todos los hilos y procesos
        self.cancellation_event.set()
        self.ffmpeg_processor.cancel_current_process()
        if self.active_subprocess_pid:
            try:
                os.kill(self.active_subprocess_pid, 9)
                print(f"DEBUG: Proceso de análisis con PID {self.active_subprocess_pid} terminado.")
            except OSError as e:
                print(f"ERROR: No se pudo terminar el proceso {self.active_subprocess_pid}: {e}")
            self.active_subprocess_pid = None

        # 2. Actualiza solo la etiqueta de progreso
        self.update_progress(0, "Operación cancelada. Puedes intentarlo de nuevo.")
        
        # 3. Restablece los botones a su estado original para permitir un nuevo intento
    ...
    def _execute_download_and_recode(self, options):
        ...
        try:
            ...
            # --- INICIO DE LA LÓGICA CORREGIDA DE NOMBRES DE ARCHIVO ---
            
            # Paso 1: Usar el ID del video para un nombre de archivo seguro y sin caracteres especiales.
            # Si no hay ID (por algún error), se usa un nombre temporal con la fecha y hora.
            safe_filename_base = self.video_id or f"temp_download_{int(time.time())}"
            
            # El título que quiere el usuario se guarda aparte para usarlo solo al final.
            user_facing_title = self.sanitize_filename(options['title'])

            # Paso 2: La plantilla de salida para yt-dlp ahora usa el nombre seguro.
            temp_filename_template = f"{safe_filename_base}_temp.%(ext)s"
            output_template = os.path.join(options["output_path"], temp_filename_template)
            ...
            if mode == "Video+Audio":
                ...
                # Para recodificar, es mejor extraer a un formato sin pérdida como WAV.
            ...
            ydl_opts = {'format': format_selector, 'outtmpl': output_template, 'postprocessors': postprocessors, 'noplaylist': True}
            ...
            if options["recode_enabled"]:
                ...
                if not codec_data: raise Exception("Codec de recodificación no encontrado.")
                ...
                # Paso 3: La recodificación también usa nombres de archivo seguros.
                final_container = options["recode_container"]
                final_safe_path = os.path.join(options["output_path"], f"{safe_filename_base}{final_container}")

                recode_opts = {
                    "input_file": downloaded_filepath,
                    "output_file": final_safe_path,
                ...
                # Paso 4: Renombrar el archivo al título final deseado. ¡Este es el paso clave!
                audio_suffix = " (Audio)" if mode == "Solo Audio" else ""
                final_user_path = os.path.join(options["output_path"], f"{user_facing_title}{audio_suffix}{final_container}")
                
                if os.path.exists(final_user_path) and final_user_path != final_path_from_recode:
                    os.remove(final_user_path) # Borra un posible archivo antiguo con el mismo nombre
                
                os.rename(final_path_from_recode, final_user_path)
                
                self.on_process_finished(True, "Recodificación completada", final_user_path)

                # Limpiar el archivo temporal de la descarga
                if os.path.exists(downloaded_filepath):
                    try: os.remove(downloaded_filepath)
                    except OSError as e: print(f"ADVERTENCIA: No se pudo eliminar el archivo temporal: {e}")

            else: # Si no había recodificación, solo renombrar el archivo descargado.
                final_ext = os.path.splitext(downloaded_filepath)[1]
                final_user_path = os.path.join(options["output_path"], f"{user_facing_title}{final_ext}")
                
                if os.path.exists(final_user_path) and final_user_path != downloaded_filepath:
                    os.remove(final_user_path)
                
                os.rename(downloaded_filepath, final_user_path)
                self.on_process_finished(True, "Descarga completada", final_user_path)

        except UserCancelledError as e:
            self.on_process_finished(False, str(e), None)

            # --- LÓGICA DE LIMPIEZA UNIFICADA ---
            print("DEBUG: Iniciando limpieza de archivos temporales por cancelación.")
            try:
                output_path = options["output_path"]
                safe_filename_base = self.video_id
                if not safe_filename_base:
                    print("DEBUG: No se encontró video_id para la limpieza.")
                    return

                # Lista de posibles archivos a eliminar
                files_to_check = []

                # 1. Archivos de la descarga original (ej: ID_temp.mp4, ID_temp.mp4.part)
                temp_download_pattern = f"{safe_filename_base}_temp"

                # 2. Archivo de la recodificación (ej: ID.mov)
                if options.get("recode_enabled"):
                    final_container = options.get("recode_container", ".tmp")
                    recode_output_filename = f"{safe_filename_base}{final_container}"
                    files_to_check.append(recode_output_filename)

                # Buscamos y eliminamos los archivos
                for filename in os.listdir(output_path):
                    # Comprueba si es el archivo de recodificación
                    if filename in files_to_check:
                        file_to_delete = os.path.join(output_path, filename)
                        try:
                            os.remove(file_to_delete)
                            print(f"DEBUG: Archivo de recodificación temporal eliminado: {file_to_delete}")
                        except OSError as cleanup_e:
                            print(f"ADVERTENCIA: No se pudo eliminar el archivo '{file_to_delete}': {cleanup_e}")

                    # Comprueba si es un archivo de la descarga original
                    if filename.startswith(temp_download_pattern):
                        file_to_delete = os.path.join(output_path, filename)
                        try:
                            os.remove(file_to_delete)
                            print(f"DEBUG: Archivo de descarga temporal eliminado: {file_to_delete}")
                        except OSError as cleanup_e:
                            print(f"ADVERTENCIA: No se pudo eliminar el archivo '{file_to_delete}': {cleanup_e}")
